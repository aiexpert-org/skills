---
name: hoard
description: "Capture a solved-problem snippet with keywords so the operator can find it again months later. When the operator solves a hard thing (a working query, a working automation, a phone script that closed a deal, a killer subject line, a prompt that finally produced the right output), this skill saves it to the operator's private playbook file with a title, tags, and the exact snippet. Trigger phrases include 'save this for later', 'hoard this', 'remember how I did this', 'add to my playbook', 'capture this snippet', 'save this to my library', 'log this working example', 'stash this', 'save the winning [X]', 'add this to my hoard', 'this worked, save it', 'remember this one', 'capture this play', 'save this prompt'. Use when the operator solves a problem in a way they'll want to reuse. Do NOT trigger for permanent business decisions (route to decision-log), for meeting notes (route to end-of-day-review), or for one-off answers that won't be reused."
---

LAST-VERIFIED: 2026-07-06. Source of truth: inspired by Simon Willison's "Hoard things you know how to do" (agentic engineering patterns, simonwillison.net/guides/agentic-engineering-patterns/hoard-things-you-know-how-to-do/). Simon's original writing is public and not licensable; this SKILL.md is an original implementation of the pattern.

This skill makes the operator's playbook compound. Operators solve the same hard problems every quarter because they never write down the solutions. The winning cold-email subject line, the SQL query that pulled the right report, the phone script that closed the deal, the prompt that finally produced the deliverable. all lost. This skill captures them the moment they work.

Output: an appended entry in `hoard/playbook.md` at the operator's HQ, plus a short chat confirmation.

## When to use

- The operator just made something work (an email, a query, a script, a workflow, a pitch, a prompt).
...
**The snippet:**
[The exact working code, copy, query, script, prompt, or process. Verbatim.]
...
**Example 2.** Operator gets a Claude prompt firing correctly after 30 minutes of iteration. Operator says "save this prompt, I'll need it again." Skill fires. Captures the prompt with a note on why it worked.
...
- `handoff`. hoarded snippets get referenced from handoff docs by path.
- `voice-corpus-builder`. hoard entries feed the voice corpus over time.

## Credit

Inspired by Simon Willison's ["Hoard things you know how to do"](https://simonwillison.net/guides/agentic-engineering-patterns/hoard-things-you-know-how-to-do/) pattern in the Agentic Engineering Patterns guide. Simon's writing is public and not licensable; this SKILL.md is an original implementation of the pattern. Credit to Simon.
