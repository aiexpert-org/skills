...
LAST-VERIFIED: 2026-07-06. Source of truth: Matt Pocock's `/handoff` skill design, MIT licensed at github.com/mattpocock/skills. Credit: Matt Pocock, aihero.dev. This Essential Bundle version is the canonical Marketplace form of Matt's original pattern, extended for operators handing off to teammates and future selves in addition to fresh coding agents.

This skill turns a scattered mid-work session into a clean handoff. Operators lose context every time they switch tasks, close a laptop, or hand something to a teammate. Matt Pocock's original `/handoff` solves this for engineers running parallel Claude Code sessions. The Essential Bundle version generalizes the pattern for every operator role.
...
- The operator is switching Claude surfaces (Claude Code to Claude Desktop, Claude to Codex).
...
**Step 5. Add suggested next skills.** From the Essential Bundle and any Persona Layer the operator has installed: what would the recipient likely reach for next? Name the skills. (Matt Pocock's original pattern.)
...
**Example 1.** Operator says "handoff, my assistant needs to run the launch follow-up while I'm at the retreat." Skill fires. Scopes to just the launch thread. Points at the launch dossier. Lists what's been sent, what's owed. Writes to `handoffs/2026-07-06-launch-followup.md` in the shared team Drive folder.
...
- `end-of-day-review`. the daily close ritual; handoff is orthogonal and can co-exist.
- `decision-log`. decisions locked in the current session get logged separately.
- `context-management`. handoff is one of three context-hygiene moves (compact, handoff, start fresh).
- `weekly-review`. closed handoffs get counted in the weekly retro.

## Credit

Matt Pocock's original `/handoff` skill is MIT licensed at [github.com/mattpocock/skills](https://github.com/mattpocock/skills). This Essential Bundle version preserves his five design decisions (temp-directory routing, suggested-next-skills pointer, no-duplicate rule, redaction, purpose-argument scoping) and generalizes the recipient set beyond fresh coding agents. Credit and thanks to Matt.
